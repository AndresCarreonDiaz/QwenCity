...
(function(){
  ...
  var PROPS=[
    // cars belong ON the asphalt (y~46); every OTHER prop must clear it — the
    // road band is y=48 ± ~2%%, and props are bottom-anchored & tall, so a base
    // just south of the road (y=50-51) makes the sprite rise up INTO the road.
    // South-side furniture therefore sits at y>=55 (clear of the y=48 asphalt).
    {slot:"car_a",x:23,y:49},{slot:"car_b",x:77,y:49},
    {slot:"car_b",x:35,y:49,flip:true,mobile:false},{slot:"car_a",x:66,y:49,flip:true,mobile:false},
    {slot:"car_c",x:59,y:49,mobile:false},{slot:"car_d",x:21,y:70,mobile:false},
    {slot:"vending",x:90,y:57,mobile:false},
    {slot:"park_statue",x:43,y:87,mobile:false},
    {slot:"bench_b",x:36,y:66,mobile:false},{slot:"bench_b",x:64,y:66,flip:true,mobile:false},
    {slot:"pigeons",x:50,y:67},{slot:"pigeons",x:28,y:44,mobile:false},
    {slot:"traffic_light",x:46,y:45},{slot:"traffic_light",x:30,y:44,mobile:false},{slot:"traffic_light",x:70,y:44,flip:true,mobile:false},
    {slot:"street_sign",x:11,y:43},{slot:"street_sign",x:89,y:43,flip:true},
    {slot:"bus_stop",x:13,y:58,mobile:false},
    {slot:"phone_booth",x:91,y:63,mobile:false},
    {slot:"hydrant",x:38,y:55},{slot:"mailbox",x:62,y:55},
    {slot:"trash_bin",x:34,y:44,mobile:false},{slot:"trash_bin",x:66,y:44,mobile:false},
    {slot:"tree_city",x:24,y:44,mobile:false},{slot:"tree_city",x:76,y:44,mobile:false},
    ...
    {slot:"planter",x:24,y:55,mobile:false},{slot:"planter",x:76,y:55,mobile:false},
    {slot:"city_bench",x:24,y:66,mobile:false},{slot:"city_bench",x:76,y:66,flip:true,mobile:false},
    {slot:"lamp",x:17,y:44},{slot:"lamp",x:33,y:44},{slot:"lamp",x:67,y:44},{slot:"lamp",x:83,y:44},
    {slot:"lamp",x:42,y:56},{slot:"lamp",x:58,y:56},
    {slot:"streetlamp_modern",x:44,y:67,mobile:false},{slot:"streetlamp_modern",x:56,y:67,mobile:false},
    {slot:"lamp",x:17,y:66,mobile:false},{slot:"lamp",x:83,y:66,mobile:false},
  ...
  function renderPanel(){
    ...
    if(!selected){
      ...
      // now with a LIVE tally of real audience-caused moments when any exist.
      var infl=snap.influences||[];
      var hookTally=infl.length?'<p style="margin:5px 0 0;color:#8fd49a"><b>'+infl.length+'</b> real moment'+(infl.length===1?'':'s')+' viewers have already changed ↓</p>':"";
      var hookHtml='<div class="hook"><div class="sh">⚡ You\\'re in the loop</div><p>Your replies change the story — <span class="stat">25% acted-on</span> vs <span class="stat">0%</span> with no audience.</p>'+hookTally+'</div>';
      ...
      // live evidence: the actual moments a viewer reply changed the story (newest first)
      var inflList=infl.length?('<div class="h" style="margin:0 0 8px">Moments the audience changed</div>'+infl.slice().reverse().map(function(m){return '<div class="hl"><b style="color:#8fd49a">@'+esc(m.handle)+'</b> nudged <b>'+esc(m.name)+'</b> — “'+esc(m.text)+'” → <span style="color:var(--dim)">'+esc(m.action)+'</span></div>';}).join("")):"";
      var whyBody=showWhy?('<div class="sci" style="margin-top:11px">'+inflList+'<p>You\\'re <b>in the loop</b>, not just watching. Click anyone and reply — your message enters the <b>same memory</b> these AI minds reason from (with a salience boost) and steers what they do next.</p>'+'<p>In a controlled test, one audience reply changed <span class="stat">25% of the town\\'s next actions</span> — vs <span class="stat">0%</span> with no audience. That open, perturbable society is the leap past a closed sim.</p>'+statLine+'<p style="margin-bottom:0"><a href="https://github.com/AndresCarreonDiaz/QwenCity" target="_blank" rel="noopener">Audience-Coupled Salience Memory ↗</a></p></div>'):"";
